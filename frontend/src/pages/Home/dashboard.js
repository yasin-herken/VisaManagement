...
import { addPrices } from '../Features/singleMultiple';
ChartJS.register(
    CategoryScale,
    LinearScale,
    PointElement,
...
function Dashboard() {
    ...
    useEffect(() => {
        const loadEntryType = async () => {
            const response = await userRequest(user.token).get("/singlemultiples", {
                params: {
                    country: user.country
                }
            });
            dispatch(addPrices(response.data))
        }
        loadEntryType();
    }, [user.country, user.token, dispatch])
    return (
